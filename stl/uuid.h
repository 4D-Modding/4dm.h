...
namespace fdm::stl
{
    ...
    public:
        ...
        inline static fdm::stl::string to_string(uuid const& u)
        {
            fdm::stl::string result(36, char());
            to_chars(u, &result[0]);
            return result;
        }
        inline static fdm::stl::string to_string_raw(uuid const& u)
        {
            fdm::stl::string result(u.begin(), u.end());
            return result;
        }
...
{
    template<>
    struct hash<fdm::stl::uuid>
    {
        std::size_t operator () (const fdm::stl::uuid& value) const noexcept
        {
            return fdm::stl::uuid::hash_value(value);
        }
    };
}