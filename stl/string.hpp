...
{
	template<class T>
	inline ostream& operator<<(ostream& os, const fdm::stl::basic_string<T>& str)
	{
		return os << str.data();
	}
	template<class T>
	inline istream& operator>>(istream& is, fdm::stl::basic_string<T>& str)
	{
		string s;
		is >> s;
		str = s;
	...
	template<class T>
	struct hash<fdm::stl::basic_string<T>>
	{
	...
	template<class T>
	inline std::string to_string(const fdm::stl::basic_string<T>& str)
	{
		ostringstream ss;
		ss << str;
		return ss.str();
	...
	template<class T>
	struct formatter<fdm::stl::basic_string<T>> : formatter<std::basic_string<T>>
	{
		auto format(const fdm::stl::basic_string<T>& v, format_context& ctx) const
		{
